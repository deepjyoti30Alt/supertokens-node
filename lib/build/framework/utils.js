...
async function inflate(stream) {
    ...
    if (encoding === "gzip" || encoding === "deflate") {
        ...
        if (!brotliDecompress) throw new Error(constants_1.BROTLI_DECOMPRESSION_ERROR_MESSAGE);
        const chunks = [];
        try {
            for (
        ...
        const compressedData = utils_1.getBuffer().concat(chunks);
        decompressedData = brotliDecompress(compressedData);
    } else {
        // Handle identity or unsupported encoding
        decompressedData = utils_1.getBuffer().concat([]);
        try {
            for (
                ...
                decompressedData = utils_1.getBuffer().concat([decompressedData, chunk]);
            }
        } catch (e_3_1) {
            e_3 = { error: e_3_1 };
        } finally {
